...
* GET home page.
*/
function getT(aux,IC){
    //gl = [0.100, 0.050, 0.025, 0.010, 0.005]
    var alpha = (1 - (IC / 100)) / 2;
    var p;
    var n = aux-1;
    if(alpha >= 0.100){
        p = 0;
    }else if(alpha < 0.100 && alpha >= 0.050){
        p = 1;
    }else if(alpha < 0.050 && alpha >= 0.025){
        p = 2;
    }else if(alpha < 0.025 && alpha >= 0.010){
        p = 3;
    }else if(alpha < 0.010 && alpha >= 0.005){
        p = 4;
    }
...
exports.execute = function(req, res){
    ...
    if( !(popN) || !(tamAmostra) || !(interCount) || !(IC)){
        ...
        var t = getT(tamAmostra,IC);
        console.log("t="+t);
        /*
        var populacao = new Array(popN);
        for (var i = 0; i < populacao.length; i++) {
            populacao[i] = randomIntInc(1,10000);
        }

        var ICs = new Array(interCount);
